...
CovarianceQuantityBase<DerivedQuantity, ParentStructure>::CovarianceQuantityBase(
    ...
    : positionCovariances(positionCovariances_), rotationCovariances(rotationCovariances_),
      ...
      showOnlyLast(uniquePrefix + "#showOnlyLast", false), showEveryNth(uniquePrefix + "#showEveryNth", 1),
...
void CovarianceQuantityBase<DerivedQuantity, ParentStructure>::drawEllipsoids(
  ...
  // Build batched geometry for all ellipsoids
  for (size_t i = 0; i < numElements; i++) {
    glm::vec3 elementPos = getPosition(i);

    // Check if we should draw this element based on filters
    bool shouldDrawThisElement = (i >= startIdx); // Respects "show only last" filter
    ...
    shouldDrawThisElement = shouldDrawThisElement && passesNthFilter;

    // Handle position ellipsoid
    if (i < positionCovariances.size() && showPositionEllipsoids.get() && shouldDrawThisElement) {
    ...
    if (i < rotationCovariances.size() && showRotationEllipsoids.get() && shouldDrawThisElement) {
      ...
      for (int axis = 0; axis < 3; axis++) {
        ...
        // Offset should be based on pose rotation, not the rotation covariance eigenvectors
        // Place the disc at maxPosRadius * 1.3 distance from center along each axis
        glm::vec3 offset = poseRotation * (offsetDir * (maxPosRadius * 1.3f));
        ...
        for (const auto& v : sphereVertices) {
          ...
          glm::vec3 rotated = params.rotation * scaled;
          // Apply pose rotation to orient disc with the pose
          glm::vec3 orientedRotated = poseRotation * rotated;
          rotEllipsoidVerts.push_back(elementPos + offset + orientedRotated);
        }
...
template <typename DerivedQuantity, typename ParentStructure>
DerivedQuantity* CovarianceQuantityBase<DerivedQuantity, ParentStructure>::setEllipsoidScale(float val) {
  ellipsoidScale = val;
  polyscope::requestRedraw();
  return derived();
}
...
template <typename DerivedQuantity, typename ParentStructure>
float CovarianceQuantityBase<DerivedQuantity, ParentStructure>::getEllipsoidScale() {
  return ellipsoidScale.get();
}
